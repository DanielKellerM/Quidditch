...
def load_spec(name):
    ...
    sh = d.get("shape", {})
    shape = (sh.get("M"), sh.get("N"), sh.get("K"))
    if any(v is None for v in shape):
        _reject(nm, "shape must give M, N, K")
    if 1 in shape:
    ...
    interchange_choices = knobs.get("l1_tiles_interchange", {}).get("choices") or [[2, 0, 1]]
    for ix in interchange_choices:
        if sorted(ix) != [0, 1, 2]:
            _reject(nm, f"l1_tiles_interchange {ix} is not a permutation of [0, 1, 2]")
