...
# QUIDDITCH_BUILD_RT selects the runtime tree (build-rt-gwaihir retargets gwaihir).
BUILD = os.environ.get("QUIDDITCH_BUILD_RT", f"{ROOT}/build-rt")

# Tool locations, env-overridable; iree-compile + the snitch-LLVM 15 fork have no in-tree path (required via env, see _require_external_tools).
...
# cfg cluster header for iree-compile; prefer the build-tree copy ninja feeds (canary needs the same one), else the committed src.
...
# Include dirs for the gemm_mod_module.c stub compile (the gemm_mod .h dir is per-config, added below).
CONST_INCLUDES = [
    f"{ROOT}/iree",
...
# Extra includes harness.c needs beyond CONST_INCLUDES.
HARNESS_INC = [
...
# Minimal module header (config-invariant query decl) so harness.o compiles pre-iree-compile.
...
def _inject(mlir_text, l1_tiles, dual_buffer, interchange=None):
    # N-dimensional: 3 tiles for matmul (M,N,K), 1 for a 1-D elementwise op.
...
def build(config, outdir, mlir_template=None, module="gemm_mod", harness_obj=None,
    ...
    config        = {"l1_tiles": [...], "dual_buffer": "true"|"false", "interchange": [...]}
                    l1_tiles/interchange are N-dim: 3 for matmul (M,N,K), 1 for a 1-D op.
    ...
    # inject lowering_config into a temp .mlir (basename kept so iree-compile names match ninja/canary).
